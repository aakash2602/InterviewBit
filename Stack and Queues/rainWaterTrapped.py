class Solution():
    ...
    #                     mode = 1
    #                     val = stack[-1]
    #                 water += (i - val - 1) * (min(A[i], A[val]) - last)
    #                 last = min (A[i], A[val])
    #                 print (f"{val}:{A[val]}:{water}:{last}")
    #                 if mode == 1:
    #                     break
    #             stack.append(i)
    #         else:
    ...
    #         while stack:
    #             val = stack.pop()
    #             water += (last_val - val - 1) * (min(A[val], A[last_val]) - last)
    #             last = min(A[val], A[last_val])
    #     return water

    # def rainWaterTrapped(self, A): ## It uses extra space
    #     n = len(A)
    #     left = [0] * n
    #     right = [0] * n
    #     for i in range(1, n):
    ...
    def rainWaterTrapped(self, A): # Constant space
        n = len(A)
        left = 0
        right = n-1
        left_max = 0
