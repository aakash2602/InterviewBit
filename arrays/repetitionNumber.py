class Solution():
    ...
    def repetitionNumber(self, A):
        n = len(A) - 1
        sqrt = int(round(n ** 0.5))
        B = [0 for i in range(sqrt + 1)]
        # print (B)
        for i in range(len(A)):
            index = int((A[i] - 1) / sqrt)
            # print (A[i])
            # print ("index: " + str(index))
            B[index] += 1
        for i in range(sqrt + 1):
            if i < sqrt and B[i] > sqrt:
                break
            elif i == sqrt:
                if B[i] > (n - sqrt * i):
                    break
        dict = {}
        for index in range((sqrt * i) + 1, sqrt * (i + 1) + 1):
            dict[index] = 0
        for i in range(len(A)):
            if A[i] in dict:
                if dict[A[i]] > 0:
